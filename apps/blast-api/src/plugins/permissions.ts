// Wave D Phase 3: per-action permission enforcement for blast-api.
//
...
const blastPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'blast-api permissions plugin registered');
  },
  { name: 'blast-permissions', dependencies: ['auth'] },
);

export default blastPermissionsPlugin;
