// Wave D Phase 3: per-action permission enforcement for blank-api.
//
...
const blankPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'blank-api permissions plugin registered');
  },
  { name: 'blank-permissions', dependencies: ['auth'] },
);

export default blankPermissionsPlugin;
