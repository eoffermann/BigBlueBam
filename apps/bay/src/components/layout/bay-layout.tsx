...
import { BaySidebar } from '@/components/layout/bay-sidebar';
...
export function BayLayout({ children, onNavigate, activeRoute }: BayLayoutProps) {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-sidebar flex flex-col">
          <BaySidebar onNavigate={onNavigate} activePage={activeRoute.page} />
        </aside>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <header className="flex items-center justify-between h-14 px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <NotificationsBell inAppPrefix="/bay/" onNavigate={onNavigate} />
              <HelpTrigger app="bay" />
              <UserMenu user={user} />
            </div>
          </header>
      ...
      <Launchpad isOpen={launchpadOpen} onClose={() => setLaunchpadOpen(false)} currentApp="bay" />
    </div>
  );
}
