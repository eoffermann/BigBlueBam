// Wave D Phase 3: per-action permission enforcement for blip-api.
//
...
const blipPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'blip-api permissions plugin registered');
  },
  { name: 'blip-permissions', dependencies: ['auth'] },
);

export default blipPermissionsPlugin;
