...
initSystemErrorReporter({ service: 'blip' });
import { App } from './app';
...
// Blip shares Bam's session and reads /b3/api/auth/me directly.
...
function describeLocation(): LocationDescriptor | undefined {
  ...
  if (!path.startsWith('/blip')) return undefined;
  return {
...
try {
  ...
  console.warn('[blip] mountBureauClient failed', err);
}
