// Wave D Phase 3: per-action permission enforcement for beacon-api.
//
...
const beaconPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'beacon-api permissions plugin registered');
  },
  { name: 'beacon-permissions', dependencies: ['auth'] },
);

export default beaconPermissionsPlugin;
