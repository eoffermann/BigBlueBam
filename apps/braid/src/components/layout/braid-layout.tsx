...
import { BraidSidebar } from '@/components/layout/braid-sidebar';
import { useAuthStore } from '@/stores/auth.store';

export type ActiveRoute =
  | { page: 'profiles' }
  | { page: 'profile-detail'; id: string; label?: string }
  | { page: 'review-queue' }
  | { page: 'survivorship' }
  | { page: 'settings' };

interface BraidLayoutProps {
  children: ReactNode;
...
const PAGE_LABELS: Record<ActiveRoute['page'], string> = {
  ...
  survivorship: 'Survivorship Rules',
  settings: 'Settings',
};

function breadcrumbsFor(route: ActiveRoute): Crumb[] {
  if (route.page === 'profiles') return [{ label: 'Profiles' }];
  if (route.page === 'profile-detail') {
    return [
      { label: 'Profiles', href: '/' },
      { label: route.label ?? 'Profile' },
    ];
  }
  return [{ label: PAGE_LABELS[route.page] }];
}

export function BraidLayout({ children, onNavigate, activeRoute }: BraidLayoutProps) {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-sidebar flex flex-col">
          <BraidSidebar onNavigate={onNavigate} activeRoute={activeRoute} />
        </aside>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <header className="flex items-center justify-between h-14 px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <NotificationsBell inAppPrefix="/braid/" onNavigate={onNavigate} />
              <HelpTrigger app="braid" />
              <UserMenu user={user} />
            </div>
          </header>
      ...
      <Launchpad isOpen={launchpadOpen} onClose={() => setLaunchpadOpen(false)} currentApp="braid" />
    </div>
  );
}
