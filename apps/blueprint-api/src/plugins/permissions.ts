// Wave D Phase 3: per-action permission enforcement for blueprint-api.
//
...
const benchPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'blueprint-api permissions plugin registered');
  },
  { name: 'blueprint-permissions', dependencies: ['auth'] },
);
