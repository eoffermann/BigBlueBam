// Wave D Phase 3: per-action permission enforcement for bond-api.
//
...
const bondPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bond-api permissions plugin registered');
  },
  { name: 'bond-permissions', dependencies: ['auth'] },
);

export default bondPermissionsPlugin;
