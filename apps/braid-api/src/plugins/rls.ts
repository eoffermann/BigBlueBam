...
/**
 * Row-level security plugin for braid-api (binding fix issue #87).
 *
 * RLS policies (migrations 0230/0231) gate braid_* rows by `current_setting('app.current_org_id')`,
 ...
 * statement's own transaction, which commits immediately), so under enforce mode every braid query
 * returned zero rows.
 *
...
const rlsPlugin = fp(async (fastify: FastifyInstance) => {
  ...
  if (!enforce) {
    fastify.log.info(
      { enforce: false },
      'braid-api rls plugin registered (advisory mode, BYPASSRLS; org GUC not bound)',
    );
    return;
  }
  ...
  fastify.log.info(
    { enforce: true },
    'braid-api rls plugin registered (NOBYPASSRLS enforce mode; per-request reserved connection binds app.current_org_id)',
  );
});
