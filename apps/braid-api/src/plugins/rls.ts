...
/**
 * Row-level security plugin for braid-api (mirrors apps/basis-api/src/plugins/rls.ts).
 *
 ...
 * policies that gate the braid_* tables (migrations 0230/0231) see the caller's
 * org. Without this the policies are inert for braid-api: in soft mode the app
 * role has BYPASSRLS so nothing changes, but the moment the platform flips
 * BBB_RLS_ENFORCE=1 (a coordinated platform posture, spec 2.5 point 8) every
 * braid query would return zero rows. Setting the GUC here keeps braid-api
 * correct under enforce mode and makes the policies actually bind. Braid does
...
const rlsPlugin = fp(async (fastify: FastifyInstance) => {
  ...
  fastify.log.info(
    { enforce },
    enforce
      ? 'braid-api rls plugin registered (NOBYPASSRLS enforce mode)'
      : 'braid-api rls plugin registered (advisory mode, BYPASSRLS)',
  );
});
