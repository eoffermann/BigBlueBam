...
class ApiClient {
  ...
  // burn-api routes are mounted under /v1 and proxied at /burn/api/ (vite rewrites
  // /burn/api -> :4022, nginx does the same in the deployed stack).
  constructor(baseUrl = '/burn/api/v1') {
    this.baseUrl = baseUrl;
  }

  private getOrgId(): string | undefined {
    try {
      const mod = (globalThis as any).__burnAuthStore;
  ...
  private async request<T>(
    ...
    if (!contentType.includes('application/json')) {
      // /v1/financials/export returns text/csv.
