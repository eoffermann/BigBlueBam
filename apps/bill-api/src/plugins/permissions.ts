// Wave D Phase 3: per-action permission enforcement for bill-api.
//
...
const billPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bill-api permissions plugin registered');
  },
  { name: 'bill-permissions', dependencies: ['auth'] },
);

export default billPermissionsPlugin;
