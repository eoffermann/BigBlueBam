...
import { BULWARK_WS_CHANNEL, bulwarkRoom } from '../lib/realtime.js';

// The /bulwark/ws realtime hub (spec 5.2). Org-scoped rooms, REFS-ONLY frames. A single
// Redis subscriber fans each published { room, event } to every local socket whose room set
// contains that room. Mirrors apps/braid-api/src/ws/handler.ts. Project-scoped frame
// filtering (SK1) is layered on in a later milestone.
...
export default async function websocketHandler(fastify: FastifyInstance) {
  ...
  fastify.get('/ws', { websocket: true }, async (socket, request) => {
    ...
    const client: ConnectedClient = {
      ...
      userId,
      orgId,
      rooms: new Set([bulwarkRoom(orgId), `bulwark:user:${userId}`]),
    };
