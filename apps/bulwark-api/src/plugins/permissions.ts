// Per-action permission enforcement for bulwark-api.
//
...
// Mirrors apps/basis-api/src/plugins/permissions.ts; the resolver call goes
// over HTTP rather than running in-process.
...
const bulwarkPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info(
      { mode: env.BBB_PERMISSIONS_ENFORCE },
      'bulwark-api permissions plugin registered',
    );
  },
  { name: 'bulwark-permissions', dependencies: ['auth'] },
);

export default bulwarkPermissionsPlugin;
