...
// Bench/LLM outage never cascades into Basis "not ready."
...
await fastify.register(metricRoutes, { prefix: '/v1' });
...
try {
  ...
  fastify.log.info(`Basis API listening on ${env.HOST}:${env.PORT}`);
} catch (err) {
