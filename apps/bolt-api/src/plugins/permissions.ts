// Wave D Phase 3: per-action permission enforcement for bolt-api.
//
...
const boltPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bolt-api permissions plugin registered');
  },
  { name: 'bolt-permissions', dependencies: ['auth'] },
);

export default boltPermissionsPlugin;
