// Wave D Phase 3: per-action permission enforcement for banter-api.
//
...
const banterPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'banter-api permissions plugin registered');
  },
  { name: 'banter-permissions', dependencies: ['auth'] },
);

export default banterPermissionsPlugin;
