// Wave D Phase 3: per-action permission enforcement for bin-api.
//
...
const binPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bin-api permissions plugin registered');
  },
  { name: 'bin-permissions', dependencies: ['auth'] },
);

export default binPermissionsPlugin;
