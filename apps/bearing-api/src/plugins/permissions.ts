// Wave D Phase 3: per-action permission enforcement for bearing-api.
//
...
const bearingPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bearing-api permissions plugin registered');
  },
  { name: 'bearing-permissions', dependencies: ['auth'] },
);

export default bearingPermissionsPlugin;
