...
// forwards to the SuperUser Log Analysis tab. Initialised as the first thing
// after imports so errors during boot are caught.
initSystemErrorReporter({ service: 'bulwark' });
import { App } from './app';
...
// Per-action permission matrix fetcher for the `useCan` hook.
// Bulwark shares Bam's session and reads /b3/api/auth/me directly.
...
function describeLocation(): LocationDescriptor | undefined {
  ...
  if (!path.startsWith('/bulwark')) return undefined;
  return {
...
try {
  ...
  console.warn('[bulwark] mountBureauClient failed', err);
}
