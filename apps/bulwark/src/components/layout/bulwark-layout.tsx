...
import { BulwarkSidebar } from '@/components/layout/bulwark-sidebar';
import { useAuthStore } from '@/stores/auth.store';

export type ActiveRoute =
  | { page: 'ledger' }
  | { page: 'contract-detail'; id: string; label?: string }
  | { page: 'radar' }
  | { page: 'notices' }
  | { page: 'compliance' }
  | { page: 'settings' };

interface BulwarkLayoutProps {
  children: ReactNode;
...
function breadcrumbsFor(route: ActiveRoute): Crumb[] {
  if (route.page === 'ledger') return [{ label: 'Obligation Ledger' }];
  if (route.page === 'contract-detail') {
    return [
      { label: 'Obligation Ledger', href: '/' },
      { label: route.label ?? 'Contract' },
    ];
  }
  return [{ label: PAGE_LABELS[route.page] }];
}

export function BulwarkLayout({ children, onNavigate, activeRoute }: BulwarkLayoutProps) {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-sidebar flex flex-col">
          <BulwarkSidebar onNavigate={onNavigate} activeRoute={activeRoute} />
        </aside>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <header className="flex items-center justify-between h-14 px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <NotificationsBell inAppPrefix="/bulwark/" onNavigate={onNavigate} />
              <HelpTrigger app="bulwark" />
              <UserMenu user={user} />
            </div>
          </header>
      ...
      <Launchpad isOpen={launchpadOpen} onClose={() => setLaunchpadOpen(false)} currentApp="bulwark" />
    </div>
  );
}
