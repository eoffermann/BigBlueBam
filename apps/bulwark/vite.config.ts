...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@bigbluebam/ui/impersonation-banner': resolve(__dirname, '../../packages/ui/impersonation-banner.tsx'),
  ...
  server: {
    ...
    proxy: {
      '/bulwark/api': {
        ...
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/bulwark\/api/, ''),
      },
      '/bulwark/ws': {
        target: 'ws://localhost:4021',
        ws: true,
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/bulwark\/ws/, '/ws'),
      },
    },
  },
});
