...
initSystemErrorReporter({ service: 'bin' });
import { App } from './app';
...
// Bin shares Bam's session and reads /b3/api/auth/me directly.
...
function describeLocation(): LocationDescriptor | undefined {
  ...
  if (!path.startsWith('/bin')) return undefined;
  return {
...
try {
  ...
  console.warn('[bin] mountBureauClient failed', err);
}
