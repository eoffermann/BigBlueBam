...
import { BinSidebar } from '@/components/layout/bin-sidebar';
...
function breadcrumbsFor(route: ActiveRoute): Crumb[] {
  switch (route.page) {
    case 'assets':
      return [{ label: 'Asset Library' }];
    case 'asset-data':
      return [{ label: 'Asset Library', href: '/' }, { label: 'Data' }];
    default:
      return [];
  }
}

export function BinLayout({ children, onNavigate, activeRoute }: BinLayoutProps) {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] flex-shrink-0 bg-sidebar flex flex-col">
          <BinSidebar onNavigate={onNavigate} activePage={activeRoute.page} />
        </aside>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <header className="flex items-center justify-between h-14 px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <NotificationsBell inAppPrefix="/bin/" onNavigate={onNavigate} />
              <HelpTrigger app="bin" />
              <UserMenu user={user} />
            </div>
          </header>
      ...
      <Launchpad isOpen={launchpadOpen} onClose={() => setLaunchpadOpen(false)} currentApp="bin" />
    </div>
  );
}
