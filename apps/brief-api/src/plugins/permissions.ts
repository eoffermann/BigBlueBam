// Wave D Phase 3: per-action permission enforcement for brief-api.
//
...
const briefPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'brief-api permissions plugin registered');
  },
  { name: 'brief-permissions', dependencies: ['auth'] },
);

export default briefPermissionsPlugin;
