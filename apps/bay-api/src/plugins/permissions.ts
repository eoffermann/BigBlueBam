// Wave D Phase 3: per-action permission enforcement for bay-api.
//
...
const bayPermissionsPlugin = fp(
  async (fastify: FastifyInstance) => {
    ...
    fastify.log.info({ mode: env.BBB_PERMISSIONS_ENFORCE }, 'bay-api permissions plugin registered');
  },
  { name: 'bay-permissions', dependencies: ['auth'] },
);

export default bayPermissionsPlugin;
