...
apps/
  ...
  mcp-server/       MCP protocol server (internal :3001, proxied at /mcp/). 885 tools across 54 modules (128 Bam core + 80 Banter + 69 Bond + 48 Brief + 47 Bill + 40 Board + 38 Blip + 38 Beacon + 37 Bureau + 36 Blueprint + 32 Bench + 30 Bearing + 28 Blast + 26 Bolt + 25 Book + 20 Blank + 20 Bursar (module registered; Launchpad/`/docs` tile withheld pending gilligan screenshots, so NOT yet in the LAUNCHPAD-derived docs catalog) + 19 Bin + 18 Burn + 16 Basis + 16 Bulwark + 14 Bay + 13 Braid + 11 Helpdesk + 36 cross-cutting platform: agent identity/audit/heartbeat, agent policies, outbound webhooks, proposals, visibility preflight, unified activity, cross-app search, fuzzy resolvers, composite views, entity links, attachments, bolt observability, dedupe, phrase counts, expertise, pattern subscriptions, ingest fingerprint; these counts are generated, run `pnpm docs:catalog`). Also hosts an internal POST /tools/call route for server-to-server invocations (Wave 0.2). confirm_action tokens are Redis-backed (mcp:confirm_token:<token> with PX TTL) with a graceful in-process fallback; the TTL is dynamic (5 min for human approvers, 60 s for agent-to-agent chains) and the register-tool wrapper enforces the §15 agent_policies kill switch + allowlist check on every service-account invocation.
  ...
  braid-api/        Braid Fastify REST API + WebSocket (internal :4020, proxied at /braid/api/, realtime at /braid/ws). Identity-resolution / golden-record customer-data platform: an AI engine clusters person-and-company rows across Bond, Bill, and Book into one confidence-scored golden profile per real-world person or company, with an evidence trail and human-in-the-loop merge review. Flagship braid_resolve(entity) returns a stable golden id for any app record. Reads source data via the shared DB; never owns or edits source rows. Added on the suite-brainstorm branch.
  braid/            Braid React SPA served by nginx at /braid/. Profile catalog, golden-profile detail with cross-app timeline, merge review queue, survivorship rules, settings.
  bulwark-api/      Bulwark Fastify REST API + WebSocket (internal :4021, proxied at /bulwark/api/, realtime at /bulwark/ws). AI contract-obligation monitor: extracts a typed, clause-cited obligation ledger from executed contracts (Bin assets, via the internal llm-provider), binds each obligation to a Bolt event pattern plus a manual-trigger affordance, and fires timezone-anchored notice deadlines against reality as it is logged. Drafts notices/compliance chases into the shared agent_proposals HITL queue; never sends unattended. Reads source data (Bin/Bond/Bill) via the shared DB; never owns or edits source rows. Added on the suite-brainstorm branch.
  bulwark/          Bulwark React SPA served by nginx at /bulwark/. Obligation ledger per job, deadline radar, drafted-notice review queue, vendor compliance matrix, settings.
  burn-api/         Burn Fastify REST API + WebSocket (internal :4022, proxied at /burn/api/, realtime at /burn/ws). AI contract-scope & margin monitor and the flagship bill-api spend gate: reads the signed SOW (a Bin asset) into a clause-cited deliverable ledger with priced envelopes, continuously attributes every logged unit of work to a Braid-resolved client in dollars (with an explicit `unscoped` bucket that is the product), and runs a deterministic, circuit-broken `burn_precheck` in front of bill-api's money-out paths before a never-in-scope charge posts. Delegates cost/bill rate resolution to bill-api's POST /internal/rates/resolve; never issues an invoice. Per-action permission enforcement is a hardcoded fail-closed boot invariant, not an env setting. Reads source data (Bin/Bond/Bill/Braid) via the shared DB; never owns or edits source rows. Added on the suite-brainstorm branch.
  burn/             Burn React SPA served by nginx at /burn/. Portfolio Board, three-bucket unscoped queue, deliverable burn-down, gate console, variance & change-order inbox, cost rates, rules, settings.
  bursar-api/       Bursar Fastify REST API + WebSocket (internal :4023, proxied at /bursar/api/, realtime at /bursar/ws). AI procurement leveling & spend-baseline monitor: derives a cited scope tree ("the ruler") from an RFP, normalizes competing vendor offers into a leveled coverage matrix that surfaces what each bid is silent about (the absence engine, with cumulative anti-blanket caps), freezes the awarded offer into an immutable clause-cited baseline, then watches observed spend (from Bill expenses, via the internal event inbox) for price drift, out-of-scope charges, unbaselined vendors, and renewal cliffs. Advisory only: the scope-gap gate never blocks a payment. Reads source data (Bin/Bond/Bill/Braid) via the shared DB; never owns or edits source rows. Per-action permission enforcement is a hardcoded fail-closed boot invariant, not an env setting. Added on the suite-brainstorm branch.
  bursar/           Bursar React SPA served by nginx at /bursar/. Vendor Portfolio (no-award-on-file is a first-class column), request Scope Tree editor with rival-promotion queue, the Leveling Matrix, the §4.7 Exclusion Diff, vendor detail with frozen baseline, Mismatch Inbox, Renewal Radar, HITL Review Queue, settings.
...
- `http://DOMAIN/braid/` serves the Braid identity-resolution SPA
- `http://DOMAIN/braid/api/` proxies to the Braid API
- `http://DOMAIN/braid/ws` proxies Braid WebSocket connections
- `http://DOMAIN/bulwark/` serves the Bulwark contract-obligation-monitor SPA
- `http://DOMAIN/bulwark/api/` proxies to the Bulwark API
- `http://DOMAIN/bulwark/ws` proxies Bulwark WebSocket connections
- `http://DOMAIN/burn/` serves the Burn contract-scope-and-margin-monitor SPA
- `http://DOMAIN/burn/api/` proxies to the Burn API
- `http://DOMAIN/burn/ws` proxies Burn WebSocket connections
- `http://DOMAIN/bursar/` serves the Bursar procurement-leveling SPA
- `http://DOMAIN/bursar/api/` proxies to the Bursar API
- `http://DOMAIN/bursar/ws` proxies Bursar WebSocket connections (scope/leveling progress, matrix updates)
...
- **Bolt event naming** is bare-name-plus-explicit-source (Wave 0.4). Every publisher calls `publishBoltEvent(eventType, source, payload, orgId, actorId?, actorType?)` from `@bigbluebam/shared` with the event name as `deal.rotting` (not `bond.deal.rotting`) and `'bond'` as the separate second argument. **The signature is positional, not an options object** (`packages/shared/src/bolt-events.ts`); this matters beyond style, because `scripts/check-bolt-catalog.mjs` validates the `(source, event_type)` pair by extracting the first two string literals from each call site, so an object-form call would both pass `undefined` for `source`/`orgId` at runtime and silently evade the CI drift guard. Migration 0120 back-fills historical rows to match, and the guard rejects source-prefixed event names.
