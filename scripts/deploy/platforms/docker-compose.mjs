...
/**
 * Render `infra/livekit/livekit.yaml` (host mode). Thin wrapper over
 * the shared renderer in scripts/livekit/render-config.mjs — the same
 * module the `livekit-config` compose init service runs in container
 * mode on every `docker compose up`. Host mode is the only place the
 * host's LAN IP can be detected (containers can't see host network
 * interfaces); the detected value is persisted to
 * infra/livekit/.lan-ip for the container-mode renders to reuse.
 *
 * Operators can re-run this anytime their network changes:
 *   node scripts/livekit/render-config.mjs && docker compose up -d --force-recreate livekit-config livekit
 */
export async function renderLivekitConfig(envConfig) {
  const { runRender } = await import('../../livekit/render-config.mjs');
...
async function deploy(envConfig, { branch = 'stable', tlsConfig = null } = {}) {
  ...
  // 1.5. Render the LiveKit config (host mode): detects + persists the
  // host's LAN IP, detects the public IP, and writes a config whose
  // connectivity block negotiates per-client reachability (full ICE +
  // STUN). The `livekit-config` compose init service re-runs the same
  // renderer on every `docker compose up`, so addresses never go stale
  // across restarts even when the deploy script isn't involved. The
  // rendered file is gitignored — it contains the deployment secret
  // and machine-specific addresses; nothing tracked ever does.
  console.log('Rendering LiveKit config...');
  try {
    await renderLivekitConfig(envConfig);
    console.log(`${check} ${dim('infra/livekit/livekit.yaml')}`);
  } catch (err) {
    console.log(red('failed'));
    console.log(dim(`  ${err.message ?? err}`));
    console.log(dim('  The livekit-config service will retry the render on `docker compose up`.'));
  }
