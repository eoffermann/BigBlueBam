...
>
> Status: design draft, hardened through adversarial review rounds 1, 2, and 3 (final cap round). FINAL, ready for app-build-from-spec. New app. Winner of the 2026-07-19 03:00 suite-brainstorm session.
...
**Invariant.** A `bulwark_notice_deadline` is a pure function of `(obligation.deadline_rule, the legal anchor time, the resolved timezone)`. Its `triggering_event_id` is a DETERMINISTIC key over source-entity identity plus a state epoch (Section 3.1, STK1/DK1), computed identically by the live drain and by `bulwark-state-reconcile` so the two paths converge on one at-most-once atom. The durable inbox (`bulwark_ingest_events`) separately deduplicates a redelivered Bolt event by `(org, source_idempotency_key)` independently of the arm key and of deadline retention, so a purge of an old discharged deadline can never resurrect a clock (Section 3.1, ST4).
...
| **Manually trigger** an obligation ("this happened, start the clock") | HITL, permission + project-scoped | `bulwark.deadline.write`; the safety net for non-calendar-unbound / no-project obligations (D8 / DI7 / DK3) |
...
| `dedup_key` | varchar(64) NOT NULL | **the stable per-obligation upsert/supersession key** (DI1/DI2/DK1), free of any non-deterministic LLM prose. For numbered clauses, `hash(normalized_clause_ref, obligation_type, ordinal)` where `ordinal` is assigned deterministically by ascending span order (`cited_span.char_start`) within a `(clause_ref, obligation_type)` group, so it is stable across the overlapping-chunk re-extraction of one clause AND across re-extraction; for null-section provisions, `hash(verified_quote_content, obligation_type)` tied to `source_doc_hash`. `trigger_description` is displayed EVIDENCE only and is NOT in the identity hash |
...
| `id` | uuid PK | |
| `organization_id` | uuid NOT NULL | FK ON DELETE CASCADE |
| `source_idempotency_key` | varchar(128) NOT NULL | the INBOX redelivery dedup atom ONLY, decoupled from the arm key (STK1): the payload `_event_id` (`event-ingestion.routes.ts:230`) when present, else the bolt event id. It dedups bolt-level redelivery, and upstream retries ONLY when the original publisher supplied a stable payload `_event_id` (bolt's own id is a `randomUUID()`, so most upstream retries are not deduped here; the deterministic arm key, not this column, is what prevents a double clock) (STJ6/STK2) |
...
| `triggering_event_id` | uuid NOT NULL | **deterministic arm key (STK1/DK1).** For event-armed (state-reflecting) bindings, `uuid5(obligation_id || <scope_fields primary entity id> || <state epoch, e.g. the source `due_at`>)`, computed IDENTICALLY on the `bulwark-fire-on-event` drain AND `bulwark-state-reconcile` so both converge on one atom; for calendar-armed, `uuid5(obligation_id || anchor_date)` (recomputed to the successor on re-point, STJ4); for manual, `uuid5(obligation_id || occurred_at)`. It is NOT derived from `source_idempotency_key` (that serves inbox redelivery dedup only) |
...
// bulwark_obligations.deadline_rule (THEME B + DI6 recurrence)
{
  ...
  "roll_forward": true,
  "grace_hours": 0,
  "recurrence": { "interval": "annual", "until": null }   // optional (DI6); absent = single occurrence; present with until:null is bounded by contract.expiry_date; both null is rejected at confirm (DK4)
}
...
3. **`0236_permissions_seed_actions_delta_022.sql`** - **generated**. The `bulwark.*` rows are hand-authored (`bulwark_` not in `APP_PREFIXES`). Strict sequence (THEME F: the codegen step is the exact Braid gap and is explicit):
   ...
   - (b) register the **12** `bulwark.*` rows (Section 10) in the `HAND_AUTHORED` array of `generate-permission-manifest.mjs`; because the loop (`generate-permission-manifest.mjs:822-831`) copies flags verbatim rather than inferring them, each row carries explicit `app:'bulwark'`, `is_read`, `is_destructive`, `requires_confirmation`, and `requires_superuser` per Section 10's values, so `bulwark.contract.delete` lands `is_destructive:true, requires_confirmation:true` and passes the BP7 manifest assertion (BK1);
   - (c) add an `if (c.id.startsWith('bulwark.')) { migrationLabel = '<this delta>'; sourceFile = 'bulwark.routes.ts'; }` provenance branch, matching the real-filename convention of the basis./braid. branches (BK2);
...
5. **Deterministic post-processing.**
   ...
   - **Normalize the clause label + compute `dedup_key` (DI1/DI2/DK1):** deterministically normalize `clause_ref` (strip prefixes/punctuation such as "S"/"Section"/"12.03", canonicalize numbering) as EVIDENCE, then compute `dedup_key` = `hash(normalized_clause_ref, obligation_type, ordinal)` for numbered clauses, where `ordinal` is assigned by ascending `cited_span.char_start` within the `(clause_ref, obligation_type)` group so it is stable across the overlapping-chunk re-extraction of one clause and across re-extraction; or `hash(verified_quote_content, obligation_type)` tied to `source_doc_hash` for null-section provisions. The free-text `trigger_description` is NOT in the hash (it drifts between chunk calls), so two extractions of the same clause collapse to one obligation and a re-extract never orphans an armed clock.
...
- **`amends_clause_ref` extraction (best-effort, uniqueness-gated, DK2):** the extraction prompt captures the base-section reference named in the amendment prose ("Section 12.3 is hereby amended/deleted"). The auto-bind fires ONLY on a UNIQUE `(clause_ref, obligation_type)` match against the base. Because one clause carries several obligations (DI1), ANY multi-match binds NOTHING and is surfaced as a mandatory reviewer-binding item (the authoritative HITL path below), so a wrong auto-bind never silently re-points or voids a clock the amendment did not touch.
- **Reviewer binding at confirm (HITL, authoritative):** the review UI lets a human bind each amendment obligation to the base obligation it supersedes, and choose an outcome: **restate** (base -> `superseded`, open deadlines **re-pointed** to the successor in one transaction, updating `obligation_id`, the recomputed `triggering_event_id`, AND `contract_id` to the successor's contract so no deadline is left split across two contracts or reaped by the base's `ON DELETE CASCADE`, DK4) or **terminate** (base -> `superseded`, open deadlines set `status='voided'` with a reason, preserving audit, DI3). A deletion clause that produces no amendment obligation is surfaced as a review item so the base is not left firing a dead clause.

**Calendar re-point key fix (STJ4).** When open deadlines are re-pointed to a successor obligation, **calendar-armed** deadlines have their `triggering_event_id` recomputed to `uuid5(successor_obligation_id || anchor_date)` (and `contract_id` updated to the successor's contract, DK4) inside the re-point transaction, guarded `ON CONFLICT (organization_id, obligation_id, triggering_event_id) DO NOTHING` against an already-armed successor calendar deadline (drop the loser). This prevents the next radar tick minting a duplicate `uuid5(successor || date)` renewal deadline. Event-armed deadlines recompute their deterministic key against the successor (`obligation_id` changes) under the same conflict guard.
...
6. **Compute the deterministic arm key (STK1/DK1):** `triggering_event_id = uuid5(obligation_id || <the entity_filter primary entity id from scope_fields> || <state epoch: the source condition's defining timestamp, e.g. the task's `due_at`>)`, the SAME formula `bulwark-state-reconcile` uses, so a dropped-then-reconciled event and a delivered event converge on one key and never mint two clocks. Insert a `bulwark_notice_deadline` `ON CONFLICT (organization_id, obligation_id, triggering_event_id) DO NOTHING` (at-most-once). Mark the inbox row `processed`. **Publish the `deadline.armed` ws frame on ANY conflict-free deadline insert regardless of arm path (BP10)** (event drain, radar calendar, manual trigger), project-scoped fan-out (Section 5.2 / SK1).

**Manual trigger, interaction defined (D8 / DI7 / DK3).** `POST /v1/obligations/:id/trigger { occurred_at }` is **restricted to obligations that are (`unbound` AND `deadline_rule.from = 'trigger_event'`, i.e. NOT calendar-derived) OR whose contract has no `project_id`** (the genuine safety-net cases). Excluding calendar-derived `renewal`/`termination` obligations (which the radar already arms on `uuid5(obligation_id || anchor_date)`) closes the manual-vs-calendar double-arm that DI7's manual-vs-event reasoning left open. It arms with `anchor_source='manual'`, `triggering_event_id = uuid5(obligation_id || occurred_at)`, and validates `occurred_at` plausibility (SH1). Authz for the no-project case is the org-membership fallback of Section 2.5(2), not owner/admin-only, so an ordinary member can actually use the safety net (SK3).
...
4. **Calendar obligations (ST7 + DI6).** For `renewal`/`termination` obligations, compute the anchor date from the contract dates and arm with `triggering_event_id = uuid5(obligation_id || anchor_date)`. When `deadline_rule.recurrence` is set (DI6), roll the anchor forward per `interval` bounded by `recurrence.until`, or by `contract.expiry_date` when `until` is null; if BOTH are null the recurrence is rejected at confirm and treated as one-shot (never unbounded, DK4), so year N and year N+1 both arm within the bound through the single unique index; a non-recurring calendar obligation arms exactly once. Publish `deadline.armed` on the conflict-free insert (BP10).
...
**`bulwark-state-reconcile` (STJ1, the braid-rescan analog).** Scheduled (every 30 min). Per org, for each armed obligation whose binding is a **state-reflecting** type (a curated set where the trigger corresponds to a queryable persistent condition: `bam:task.overdue` reflects a task past `due_at` still `open`; a Bill invoice overdue; a compliance doc near expiry), re-query the source table (directly via `DATABASE_URL`) for rows currently satisfying the condition AND matching the obligation's `entity_filter` scope, and arm any deadline not already armed. **Idempotency across its own 30-min runs comes from the deterministic arm key (STK1/STK3), NOT from a watermark:** because state-reconcile computes the SAME `triggering_event_id = uuid5(obligation_id || entity id || state epoch)` as the live drain, an overdue task that persists for days re-hashes to the same key every run and the `ON CONFLICT DO NOTHING` collapses it to one deadline (a source `updated_at` watermark alone would not, since the condition persists). A genuinely new window (the source `due_at` changed) yields a new epoch and legitimately arms a fresh clock; a discharged deadline is NOT re-armed while the source condition is unchanged, because the discharged row already occupies that deterministic key. This recovers a dispatch dropped on the bolt-api hop for the majority of beachhead bindings, because they reflect state rather than a one-shot event. Genuinely transient bindings (no persistent source row) are NOT covered here and depend on the bolt-api sending-end outbox (Open Question 1). Per-N progress logging.
...
| GET | `/v1/deadlines/:id` | Detail (radar fields); the `notice_draft` body is included ONLY for `bulwark.notice.draft` holders (SK2) | `bulwark.deadline.read`; **project-scoped (SH3)**; member-tier `deadline.read` callers get radar fields WITHOUT the notice body |
...
Redis-PubSub, refs-only, **project-scoped fan-out, NOT org-wide (SK1)**. The frames carry contract/deadline metadata (existence, severity, `contract_id`, `due_at`) that the REST read floor restricts, so a frame is delivered to a subscriber ONLY when that subscriber passes `isProjectMember` on the frame's `contract.project_id` (org-admin override), the SAME predicate the REST query builder uses (`visibility.service.ts:192`). Rooms are keyed per `(org, project)` and the fan-out drops any frame for a contract whose project the subscriber is not a member of; frame types that carry `contract.read`/`obligation.read`-floored information are additionally restricted to owner/admin subscribers. This mirrors how Board and Beacon scope ws rooms below org, and closes the read side that 2.5(2) flagged as the write-discoverability vector. Frames: `deadline.armed { deadline_id, obligation_id, due_at }` (published on ANY conflict-free deadline insert regardless of arm path, BP10), `waiver.risk_detected { risk_id, severity, contract_id }`, `notice.drafted { deadline_id, proposal_id }`, `compliance.expiring { compliance_doc_id, vendor_tier_id }`. No clause text or PII. Modeled on `apps/basis-api` / `apps/bin-api` ws routes + Redis PubSub cross-instance broadcast.
...
- **Unit (Vitest, `@bigbluebam/db-stubs`, basis safety-suite precedent commit `7587872c`):**
  ...
  - **deterministic arm-key convergence (STK1/DK1/STK3):** the fire-on-event drain and `bulwark-state-reconcile` for the SAME overdue task yield exactly ONE deadline (identical `triggering_event_id`); a discharged deadline is NOT re-armed while the source condition is unchanged; a changed source `due_at` (new epoch) arms a fresh clock; two overlapping-chunk extractions of one clause collapse to one obligation and a re-extract does not orphan an armed clock (DK1 ordinal key, trigger prose excluded).
  - **amendment auto-bind uniqueness (DK2):** an `amends_clause_ref` matching multiple base obligations in one clause binds NOTHING and surfaces a mandatory reviewer-binding item; restate re-point updates `contract_id` to the successor (DK4) so the base cascade cannot reap it.
  - **manual-vs-calendar (DK3):** a calendar-derived `renewal` obligation on a project contract is rejected by `POST /trigger` (excluded from the manual path), so no double-arm against the radar; a recurrence with both `until` and `expiry_date` null is rejected at confirm (DK4).
  - **ws project-scoping (SK1):** a non-project member does NOT receive `waiver.risk_detected`/`deadline.armed` frames for a contract they cannot read; a `contract.read`-floored frame reaches only owner/admin subscribers.
  - **notice-body floor (SK2):** a member-tier `deadline.read` caller GETs `/deadlines/:id` WITHOUT `notice_draft.body_markdown`; a `bulwark.notice.draft` holder gets it.
  - **no-project authz (SK3):** an ordinary org member (not admin) can `POST /trigger` on a null-`project_id` contract via the org-membership fallback; reads apply the same fallback symmetrically.
...
- **Compose (`docker-compose.yml` worker service):** confirm `BBB_API_INTERNAL_URL: http://api:4000` is present (already added by the Braid build at `docker-compose.yml:258`; no edit needed if so, IK1).
...
- **MCP wiring (IN1):** add `BULWARK_API_URL` to `mcp-server.env.optional` in `services.mjs` AND to the docker-compose `mcp-server` block (mirroring the `BRAID_API_URL: http://braid-api:4020/v1` line at `docker-compose.yml:188`), value `http://bulwark-api:4021/v1`, plus an env-hint. **Backfill the pre-existing bug (NOTE: two of these three are unrelated pre-existing fixes, not Bulwark scope creep, IK2):** `BASIS_API_URL` and `BRAID_API_URL` are in `docker-compose.yml` but MISSING from `mcp-server.env.optional`; add all three (`BULWARK_API_URL` is the Bulwark one). **`mcp-server.needs` (IN4):** do NOT add `bulwark-api`, matching braid/basis. Register `bulwark-tools.ts` in the MCP bootstrap.
...
- `GET /v1/deadlines/:id` -> skip: resolver-done-internally (`bulwark_check_notice_risk` + `bulwark_list_deadlines` surface deadline data; the `notice_draft` body is a UI-only read gated behind `bulwark.notice.draft`, owner/admin floor, SK2).
...
Final cap round. Best-practices and infrastructure came back clean of majors. Every finding accepted or accepted-with-adaptation; none rejected. Round-2 verified fixes were not re-touched. After this fold the spec is FINAL and hands off to app-build-from-spec. Counts unchanged: 15 tools, 12 permissions, 6 events, 7 jobs, 9 tables. Dispositions:

**The unifying fix: deterministic arm keys**
- [stability] STK1 ACCEPT (BLOCKER): the deadline arm key is now DETERMINISTIC on source-entity identity plus a state epoch (`uuid5(obligation_id || entity id || due_at)`), computed IDENTICALLY by the `bulwark-fire-on-event` drain and `bulwark-state-reconcile`, so a delivered event and a dropped-then-reconciled event converge on one at-most-once atom; `source_idempotency_key` is decoupled and serves inbox redelivery dedup only. This kills the double-arm (both paths run) and the re-arm-after-discharge that the round-2 key spaces allowed. Sections 2.1, 3.1, 4.2, 4.5, 8. (from review round 3)
- [design] DK1 ACCEPT (MAJOR): `dedup_key` for numbered clauses drops the non-deterministic LLM `trigger_description` and keys on `hash(normalized_clause_ref, obligation_type, ordinal)` with `ordinal` by `cited_span.char_start`, stable across overlapping-chunk re-extraction and re-extraction; trigger prose is evidence only. This kills the normal-run duplicate obligation and the re-extract orphaning of an armed clock. Sections 3.1, 4.1, 8. (from review round 3)

**Design (remaining)**
- [design] DK2 ACCEPT (MAJOR): the `amends_clause_ref` auto-bind fires only on a UNIQUE `(clause_ref, obligation_type)` match; any multi-match binds nothing and becomes a mandatory reviewer-binding item, so a wrong auto-bind never corrupts a live clock in the multi-obligation-per-clause case DI1 models. Sections 4.1, 8. (from review round 3)
- [design] DK3 ACCEPT (MAJOR): the manual-trigger predicate is tightened to (`unbound` AND `deadline_rule.from='trigger_event'`) OR no-project, excluding calendar-derived `renewal`/`termination` from the manual path so they cannot double-arm the radar's calendar clock. Sections 2.2, 4.2, 5.1, 8. (from review round 3)
- [design] DK4 ACCEPT (MINOR): the restate re-point updates the deadline's `contract_id` to the successor's contract (no split-contract deadline, no base-cascade reap); `recurrence.until:null` is bounded by `contract.expiry_date`, and both-null is rejected at confirm (never unbounded). Sections 3.3, 4.1, 4.3, 8. (from review round 3)

**Security**
- [security] SK1 ACCEPT (MAJOR): `/bulwark/ws` frames are project-scoped (per-`(org, project)` rooms, `isProjectMember` + org-admin override on fan-out, `contract.read`/`obligation.read`-floored frame types restricted to owner/admin), mirroring Board/Beacon, closing the org-wide BI leak of clock existence/severity/`contract_id`/`due_at`. Sections 2.5(1), 4.2, 5.2, 8. (from review round 3)
- [security] SK2 ACCEPT (MAJOR, option a): the `notice_draft` body is gated behind `bulwark.notice.draft` (owner/admin floor); `GET /v1/deadlines/:id` splits the response so member-tier `deadline.read` callers get radar fields WITHOUT the LLM notice prose, resolving the contradiction with the twice-claimed floor. Sections 2.2, 5.1, 10, 8. (from review round 3)
- [security] SK3 ACCEPT (MINOR): null-`project_id` contracts use an org-membership fallback for reads AND writes (symmetric in the query builder and mutating guard), so the manual-trigger safety net is usable by ordinary members rather than silently owner/admin-only. Sections 2.5(2), 4.2, 8. (from review round 3)

**Stability (minors/nit)**
- [stability] STK2 ACCEPT (MINOR): reworded `source_idempotency_key` to "dedups bolt-level redelivery, and upstream retries only when the publisher supplies a stable payload `_event_id`" (bolt's own id is a `randomUUID()`). Section 3.1. (from review round 3)
- [stability] STK3 ACCEPT (MINOR): state-reconcile idempotency across its own runs is explicitly the deterministic arm key (STK1), not a watermark; called out so an implementer does not lean on `updated_at` alone. Section 4.5, 8. (from review round 3)
- [stability] STK4 ACCEPT (NIT): noted the Redis gate is ADVISORY and `bulwark_obligations` is authoritative (a rolled-back arm's stale `SADD` self-heals on rebuild), so nobody hardens it into a two-phase commit. Section 4.5. (from review round 3)

**Best-practices (minors, no major)**
- [best-practices] BK1 ACCEPT (MINOR): 3.4 step (b) now lists the full explicit flag set (`app`, `is_read`, `is_destructive`, `requires_confirmation`, `requires_superuser`) because the `HAND_AUTHORED` loop copies flags verbatim; `bulwark.contract.delete` lands the destructive flags and passes BP7. Section 3.4. (from review round 3)
- [best-practices] BK2 ACCEPT (NIT): 3.4 step (c) provenance `sourceFile = 'bulwark.routes.ts'`, matching the basis./braid. branch convention. Section 3.4. (from review round 3)

**Infrastructure (nits, no major)**
- [infrastructure] IK1 ACCEPT (NIT): 9.2 worker compose bullet reworded "confirm present" (already added by Braid at `docker-compose.yml:258`). Section 9.2. (from review round 3)
- [infrastructure] IK2 ACCEPT (NIT): 9.5 notes two of the three MCP-URL backfills (`BASIS_API_URL`/`BRAID_API_URL`) are unrelated pre-existing fixes, not Bulwark scope creep. Section 9.5. (from review round 3)
