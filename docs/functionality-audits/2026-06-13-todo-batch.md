...
- **Frontend SPAs (banter/bond/blast):** they build into the single `frontend` nginx image (no per-SPA compose service). They are production-build-verified; **deploying them for the visual UI smoke is the operator step:** `docker compose build frontend && docker compose up -d --force-recreate frontend`. The UI fixes (#1 menu, #3 add-many dialog, #4 Bond Analytics, #5 Blast SMTP) need a browser to smoke — exact steps are in each commit's body and the session report.
