...
app: bill
generated: "2026-06-23T06:01:51.772Z"
---
...
![Recurring](screenshots/light/07-recurring.png)
...
| `bill_cancel_recurring_invoice` | Cancel a recurring schedule permanently. Already-generated invoices are unaffected. | `recurring_id` |
...
| `bill_generate_recurring_invoice_now` | Generate an invoice from a recurring schedule immediately, without waiting for the next scheduled run. Advances next_run_at by one cadence step. Returns the new invoice id, number, and status. | `recurring_id` |
...
| `bill_list_recurring_invoices` | List recurring/subscription invoice schedules for the org, optionally filtered by status (active, paused, cancelled). | `status` |
| `bill_pause_recurring_invoice` | Pause a recurring schedule — stops invoice generation until resumed. | `recurring_id` |
...
| `bill_resume_recurring_invoice` | Resume a paused recurring schedule — generation continues at next_run_at. | `recurring_id` |
...
| `bill_update_recurring_invoice` | Update a recurring invoice schedule. Provide only the fields to change. Passing line_items replaces the whole template. | `recurring_id`, `cadence`, `auto_finalize`, `currency`, `tax_rate`, `discount_amount`, `payment_terms_days`, `start_date`, `end_date`, `next_run_at`, `notes`, `line_items` |
