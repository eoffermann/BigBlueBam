---
title: "Blueprint"
app: blueprint
generated: "2026-06-17T20:37:35.166Z"
---

# Blueprint


Diagrams that are data, not drawings - every node and edge is a real, auditable object your team and your AI agents can read, edit, version, and turn into work.

- Typed node/edge graph in relational tables: every structural change is an auditable API call and an MCP tool, so an agent builds or rewrites a diagram with the same authority as a human.
- One canvas for flowcharts, graphs, sequence, class, and mind maps, with server-side ELK auto-layout, snapshots, and Mermaid and JSON import/export.
- Round-trips with Bam: generate a diagram from a project's tasks, promote a node or a whole graph into tasks, and keep nodes and tasks in two-way sync.
- An agent can read a process doc and emit a fully wired, laid-out diagram in a single call, under the platform's agent policies, audit trail, and human-approval queue.

## See It in Action


![Diagram List](screenshots/light/01-diagram-list.png)


![Diagram Editor](screenshots/light/02-diagram-editor.png)

![New Diagram Dialog](screenshots/light/03-new-diagram-dialog.png)

---

Part of the [BigBlueBam](/) productivity suite.
