...
carry a full task record: assignee, priority, story points, due dates, subtasks,
comments, attachments, time entries, and custom fields.
...
a sprint with the same authority a human has in the UI.
...
project list), **My Work** (your personal task queues), and the **Projects** list
...
![Bam Kanban board](screenshots/light/01-board.png)
...
None), **Move to phase**, **Set state**, **Assign to...** (Unassigned plus
...
To edit a field, change its control in the sidebar; the drawer saves the change as
you go. The description auto-saves shortly after you stop typing.
...
![Task detail drawer](screenshots/light/03-task-detail.png)
...
To change view, click the matching icon in the view switcher. Workload shows load
per user; clicking a user there filters the board to that user.
...
only) or add the user to projects from their **Projects** tab.

![People management](screenshots/light/04-people.png)

### My Work

My Work is your personal queue across every project.

To open it, click **My Work** in the sidebar. The page groups your tasks into
**Overdue**, **Due This Week**, **In Progress**, and **All My Tasks**.

### Settings

Settings control your profile and the org's configuration.

To open settings, go to **/settings**. The tabs are **Profile**, **Appearance**
...
![Project and app settings](screenshots/light/05-settings.png)
...
well as UUIDs. Bam exposes tools across nine areas:
...
tool stages the action and returns a token, and a second call confirms it. Tokens
are time-limited. The idempotent `task_upsert_by_external_id` keys on the project
plus an external id and emits a `task.upserted` event with a `created` flag, so
agents can sync from an external system without creating duplicates. When reviewing
agent work, watch the project audit log and the task Activity tab.
...
2. Fill in **Title**, then optionally **Phase**, **Priority**, **Story Points**,
   **Assignee**, **Due Date**, and **Labels**, and click **Create Task**.
...
2. Scan the **Overdue**, **Due This Week**, **In Progress**, and **All My Tasks**
   groups.
3. Click any task to open its drawer and act on it.

**Result:** You have a single, cross-project view of your own work.
...
**Related:** Saved views, Alternate views.
...
**Related:** Share to Banter; the `bam_get_task_by_human_id` tool.
