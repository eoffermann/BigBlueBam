...
- **Agents are architecture, not a sidebar.** AI agents act on the same surfaces, permissions, and audit log as people via the 847-tool MCP surface; agency is the design center, not an add-on chat box.
...
The live catalog is **847 tools** in `apps/mcp-server/src/tools/*.ts`. The canonical total is derived by the generator `scripts/docs/build-docs-catalog.mjs` (run via `pnpm docs:catalog`), which scans the `registerTool(` source and writes `site/src/content/docs-catalog.generated.json`; the marketing site reads that file through `site/src/content/catalog-stats.ts` (`TOTAL_MCP_TOOLS`) so the number can never drift. Note: `docs/reference/mcp-server.md` still claims "340 tools across 43 modules" - that figure and its per-module tables are stale and should not be trusted over the generated catalog.
...
| `mcp-server.md` | 965 | MCP transport model, what MCP is, per-module tool listing | **Stale counts:** claims "340 tools across 43 modules" vs. the canonical generated catalog's 847; **omits Bureau** entirely. |
...
- **Tool/module counts diverge.** `mcp-server.md` (340/43, no Bureau) lags the canonical generated catalog (847 tools); trust the generated catalog (`site/src/content/docs-catalog.generated.json`, via `pnpm docs:catalog`) and the catalog code, not `mcp-server.md`, for current numbers.
...
- `docs/reference/mcp-server.md` - MCP server documentation with a module-by-module tool listing. Note a count discrepancy: this doc states "340 tools across 43 modules," while the canonical generated catalog (`site/src/content/docs-catalog.generated.json`, via `pnpm docs:catalog`) totals 847 - the doc's totals are stale; trust the generated catalog over it when it matters.
