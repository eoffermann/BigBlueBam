...
| | **2026-06-12 status note:** the P-4 *storage half* shipped (table, service with inheritance resolution, REST routes, dialog UI) but the *enforcement half* never did — no token-mint or ring path reads project overrides, so the settings were write-only. The per-project "Calling Settings" menu entry + dialog were therefore **removed from the Bam board UI** (operators were configuring values that did nothing). The schema, service, and routes remain for whenever enforcement lands; the live calling config homes are the org page (`/banter/admin/calling-settings`) and the SuperUser platform page (`/b3/superuser/platform-calling`). | | |
