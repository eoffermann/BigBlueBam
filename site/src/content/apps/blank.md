---
title: "Blank (Forms)"
app: blank
generated: "2026-06-24T21:06:05.368Z"
---

# Blank (Forms)


Build forms and surveys with a visual editor, publish them to a public link, and watch the responses roll in.

- Drag-and-drop form builder with over 20 field types, real file uploads, and multi-page support
- Public form pages that work for anonymous respondents, with org and project scoping, sign-in, and allowed-domain gating when you need it
- Email notifications on every submission, with an optional post to a Banter channel
- Response analytics with totals, a 30-day submission trend, and per-field breakdowns, plus one-click CSV export
- AI agents can generate a form from a plain-language description, publish it, and summarize the responses for you
- Reach your team without scheduling: the persistent Bureau presence dock lets you see who is around and start a voice or video huddle from anywhere in Blank

## See It in Action


![Form List](screenshots/light/01-form-list.png)


![Form Builder](screenshots/light/02-form-builder.png)

![Form Preview](screenshots/light/03-form-preview.png)

---

Part of the [BigBlueBam](/) productivity suite.
