...
- [ ] **The repo `README.md` app catalog includes the app.** The README is the front door
      for anyone landing on GitHub, so it must present the whole suite without a trip to the
      marketing site. The app-catalog region (between the `<!-- AUTODOCS:APP_SECTIONS ... -->`
      markers) is GENERATED by `scripts/docs/publish.mjs` - regenerate it (`pnpm docs:readme`)
      so the new app appears with its description, a screencap, and a link to its
      `docs/apps/<app>/help.md`. Confirm `pnpm docs:readme:check` exits 0 and that the
      hand-written app-count and MCP-tool-count figures in the README prose were reconciled to
      the real totals (do not leave "N apps"/"NNN tools" copy stale). An incomplete/withheld
      app (one not in the Launchpad) must NOT appear in the README catalog.
