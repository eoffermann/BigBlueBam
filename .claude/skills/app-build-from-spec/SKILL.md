...
- **Repo README:** regenerate the README app catalog so GitHub visitors see the whole suite
  without the marketing site. The `<!-- AUTODOCS:APP_SECTIONS ... -->` region is generated by
  `scripts/docs/publish.mjs` (`pnpm docs:readme`); after it runs the new app must appear with
  a description, a screencap, and a link to its `docs/apps/<app>/help.md`. Reconcile the
  hand-written app-count and MCP-tool-count figures in the README prose to the real totals.
  `pnpm docs:readme:check` must exit 0 (it is CI-enforced). A withheld/incomplete app must
  NOT be added to the README catalog (same rule as the Launchpad).
